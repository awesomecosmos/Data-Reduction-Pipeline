...
for key,value in target_names_dict.items():
    exptimes_lst.append([key,exptime_checker(value)])   
...
DARK_cal_imgs = ImageFileCollection(DARK_cal_path,glob_exclude=['/*-0-*.fit','/*-1-*.fit'])
...
# # will be calibrated. This is because sometimes the biases can be either 0s or 1s.
# good_DARK_cal_files = []
# for DARK_cal_file in DARK_cal_files:
#     hdu1 = fits.open(DARK_cal_file)
#     dark_exptime = hdu1[0].header['EXPTIME']
#     if dark_exptime >1:
#         good_DARK_cal_files.append(DARK_cal_file)

# separating the remaining DARK files into their chips
...
# selecting images and excluding non-science images
good_flat_files = []
flats_to_include = ['/*-1.fit','/*-2.fit','/*-3.fit','/*-4.fit','/*-5.fit',
              '/*-6.fit','/*-7.fit','/*-8.fit','/*-9.fit','/*-10.fit']
for i in flats_to_include:
    good_flat_file = glob.glob("C:\\Users\\ave41\\OneDrive - University of Canterbury\\Master's 2021\\ASTR480 Research\\ASTR480 Code\\Data Reduction Pipeline\\ObsData_v4\\FLAT" 
                          + i)
...
for key, value in target_names_dict.items():
    ...
    for a_index,ALERT_chips in enumerate(ALERT_chips_files):
        ...
        for ALERT_file in ALERT_chips:
                hdu1 = fits.open(ALERT_file)
                # extracting header data for later saving purposes
                al_file_name = hdu1[0].header['RUN'].strip(' ')
                alert_exptime = hdu1[0].header['EXPTIME']
                al_obs_set = hdu1[0].header['SET'].strip(' ')
                al_chip_num = hdu1[0].header['CHIP']
                ...
                #     else:
                #         # Find the correct dark exposure
                #         MFLAT_exptime = find_nearest_dark_exposure(mdark_ccd,actual_exposure_times)
                #         MFLAT_to_divide = combined_darks[MFLAT_exptime]
