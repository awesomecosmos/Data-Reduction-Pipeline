...
for index,DARK_chips in enumerate(DARK_chips_files):
    # getting some numbers for display/saving purposes later
    chip_num = chip_num_extractor(DARK_chips_files[index][0])
    num_of_biases = len(DARK_chips_files[0])
    #exptime = 

    # converting each DARKS file to a fits array
    DARK_fits = [fits.getdata(DARK_file) for DARK_file in DARK_chips]
    # converting each DARKS array to a CCDData object
    DARK_ccd = [CCDData(DARK_fit,unit=u.adu) for DARK_fit in DARK_fits]

     
    for j in range(len(DARK_ccd)):
        # Subtract bias from each Dark
        ccd = ccdp.subtract_bias(DARK_ccd[j], master_bias)
        # Save the result
        filename = DARK_chips[j][-18:-4]
        ccd.write(DARK_cal_path / "calibrated_dark_{}.fit".format(filename),overwrite=True)
...
for index,DARK_chips in enumerate(DARK_cal_chips_files):
    # getting some numbers for display/saving purposes later
    chip_num = chip_num_extractor(DARK_cal_chips_files[index][0])
    num_of_biases = len(DARK_cal_chips_files[0])
    
    # converting each DARKS file to a fits array
    DARK_cal_fits = [fits.getdata(DARK_cal_file) for DARK_cal_file in DARK_chips]
    # converting each DARKS array to a CCDData object
    DARK_cal_ccd = [CCDData(DARK_cal_fit,unit=u.adu) for DARK_cal_fit in DARK_cal_fits]
    
    # combining all the darks together
    master_dark = ccdp.combine(DARK_cal_ccd,unit=u.adu,
                              method='average',
                              sigma_clip=True, 
                              sigma_clip_low_thresh=5, 
                              sigma_clip_high_thresh=5,
    ...
    master_dark.write(MDARK_path / 'mdark_chip{}.fit'.format(chip_num),overwrite=True)

    # plotting single dark compared to combined dark
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20, 10))
    show_image(DARK_cal_ccd[0], cmap='gray', ax=ax1, fig=fig, percl=90)
